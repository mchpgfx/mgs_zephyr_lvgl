...
/**
 * @brief Allocator hooks for the GPU working heap.
 *
 * Implemented by the BSP/consumer. Every nano2D internal allocation (the device
 * context, per-commit command buffers, GPU command/queue nodes) funnels through
 * these. The consumer backs them with a single contiguous, identity-mapped
 * region handed to n2d_init(). On this port that region is non-cached, so the
 * GPU's DMA observes CPU writes without any cache maintenance.
 *
 *   heap_init - initialise the allocator over [base, base+size); called once
 *               from n2d_init() before any allocate.
 *   allocate  - return a block of @p size bytes, or NULL when exhausted.
 *   free      - release a block previously returned by allocate.
 */
 ...
 *
 * Implemented by the BSP/consumer. Orders prior writes to GPU command memory
 * before the subsequent GPU register write that starts execution. Required even
 * when that memory is non-cached: it guarantees the stores are globally
 * observable (drained from any write buffer) before the GPU's AXI master
 * fetches them.
 */
